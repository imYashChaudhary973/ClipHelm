...
The Source step accepts MP4 and MOV, plus MKV when AVFoundation can decode it. Choose a file or drop it into the window. Direct video URL import is temporarily disabled because its download transport cannot yet prevent DNS rebinding. YouTube import accepts public video links for content you own or may process. It requires `yt-dlp` installed at `/opt/homebrew/bin/yt-dlp` or `/usr/local/bin/yt-dlp`; FFmpeg is needed when its best video and audio streams require merging. ClipHelm does not pass cookies or sign-in credentials to it. A failed or protected YouTube link stays unavailable rather than bypassing access controls.

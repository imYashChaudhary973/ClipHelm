...
The Source step accepts MP4 and MOV, plus MKV when AVFoundation can decode it. Choose a file or drop it into the window. Direct video URL import is temporarily disabled because its download transport cannot yet prevent DNS rebinding. YouTube import accepts public video links for content you own or may process. On each Mac that will import YouTube videos, install `yt-dlp` first (`brew install yt-dlp`); ClipHelm looks in `/opt/homebrew/bin` and `/usr/local/bin`. Install FFmpeg (`brew install ffmpeg`) for the highest-quality separate video/audio streams. Without FFmpeg, ClipHelm requests a single MP4 stream, which can have lower resolution or be unavailable for some videos. The app does not pass cookies or sign-in credentials to the downloader. A failed or protected link stays unavailable rather than bypassing access controls.

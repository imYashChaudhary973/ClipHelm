...
`ClipPlanner` validates the proposal, optional `AIEditIntent`, local analysis, and optional transcript against the source asset. It uses the user configuration for layout and audio, conservatively cuts high-confidence local pause signals, and preserves classified demos when requested. Smart Auto Frame uses local scene-aware animated crops; Full Frame uses a centered static fill crop. One retained segment may contain several crop paths because scene cuts and long shots are separate trajectories. Optional validated AI Vision content labels may guide local focus selection but never provide crop coordinates. Captions are emitted only for retained transcript words. Filler-word edits remain later-phase work.

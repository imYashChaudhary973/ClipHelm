...
`ClipProposal` and `AIEditIntent` may suggest meaning and intervals. The planner validates them against actual asset metadata and local evidence. User framing configuration takes precedence over an intent's framing preference. Unknown JSON fields are ignored by Swift decoding and have no executable sink. A proposal outside its asset or an intent outside its proposal is rejected. `ClipRenderer` receives only a validated `ClipHelmEditSpec` and an independently resolved source URL; AI output cannot supply a path, command, URL, or filter to AVFoundation.

Phase 15's results editor changes one clip at a time. It keeps the original `ClipProposal` and stable `ClipID`, and bounds user trim to that proposal. Trim, pacing, and framing mode changes call `ClipPlanner` again with local analysis; caption style and Full Frame focus changes update the existing validated spec so its cuts and other framing evidence remain stable. Full Frame focus sliders create validated static crop rectangles. Regenerate Framing resets to Smart Auto Frame and recalculates locally; repeated runs with unchanged evidence may produce the same framing. Revision renders new preview/final files first, then atomically replaces the project clip record. Rename needs no source or render; visual edits require reattaching the original after relaunch. No new OpenRouter inference is triggered by these edits.

...
Dependencies point downward. `ClipHelmCore`, `ClipHelmSecurity`, `ClipHelmOpenRouter`, and the app shell are implemented. Other modules are design boundaries for later phases.
...
The Phase 1 app saves draft choices. Phase 2 adds the Keychain vault, sanitized OpenRouter gateway, settings, and model registry. Media and editing remain later phases.

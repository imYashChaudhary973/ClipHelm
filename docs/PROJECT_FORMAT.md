...
Saves use an atomic replacement after validation. On launch, the store checks manifest version, transcript bounds, clip specs, IDs, and relative file names. Completed transcripts and clip records survive relaunch. Local originals can be located again; remote users re-enter an authorized link because downloaded media is temporary. Cache removal cannot remove the manifest, original media, accepted edits, or exports. Interrupted renders use disposable temporary output; a failed processing batch removes only its new files. Retry reuses a saved transcript and versioned analysis cache when valid, then writes new uniquely named clips.

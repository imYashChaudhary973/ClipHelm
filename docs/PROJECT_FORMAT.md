...
Saves use an atomic replacement after validation. On launch, the store checks manifest version, transcript bounds, clip specs, IDs, and relative file names. Completed transcripts and clip records survive relaunch. For a valid project, startup removes unreferenced app-generated MP4s and render sidecars left by an interrupted run; referenced files and unknown names remain. Cleanup does not follow a symlinked Exports directory or run against an invalid manifest. If a referenced file is missing, the clip record remains and the results view reports the missing output. Local originals can be located again; authorized YouTube users re-enter a link because downloaded media is temporary. Existing direct-URL projects remain readable, but source reattachment is unavailable while that import path is gated. Cache removal cannot remove the manifest, original media, accepted edits, or exports. Interrupted renders use disposable temporary output; a failed processing batch removes only its new files. Retry reuses a saved transcript and versioned analysis cache when valid, then writes new uniquely named clips.

The original source file is never modified. Generated previews and finals remain in the project package. The results view copies selected final MP4s to a user-chosen folder using safe names, without overwriting files already there; an interrupted or failed export removes only copies created by that export. Editing renders new files before atomically replacing a clip record, then removes superseded generated files. Deleting a clip removes its record and generated files; externally exported copies remain. Source access still requires the user after relaunch. A failed processing run removes its new generated files; accepted clips from prior runs stay intact.
